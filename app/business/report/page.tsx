    ...
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <main className="max-w-4xl mx-auto px-6 sm:px-10 py-10 print:py-2">
        ...
        <div className="flex items-start justify-between gap-6 pb-6 border-b border-slate-200">
          <div>
            <p className="text-[10px] tracking-[0.3em] uppercase text-slate-400 font-bold mb-2">Team Impact Report</p>
        ...
        <section className="mt-8">
          <h2 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">At a glance</h2>
        ...
        {/* Skill gaps */}
        <section className="mt-8">
          <h2 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Skill gaps across the team</h2>
          {membersAssessed === 0 ? (
              ...
              <div className="space-y-2">
                {topWeak.map((w) => (
                  <div key={w.topic} className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-slate-400 tabular-nums w-20 text-right">{w.count}/{membersAssessed} weak</span>
                  </div>
                ))}
              </div>
              {topStrong.length > 0 && (
                <div className="mt-4">
                  <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Team strengths</p>
        ...
        {/* Roster */}
        <section className="mt-8">
          <h2 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Per-person progress</h2>
          {roster.length === 0 ? (
            <p className="text-sm text-slate-500">No team members have joined yet.</p>
          ) : (
            <table className="w-full text-sm border border-slate-200 rounded-xl overflow-hidden">
              <thead>
                <tr className="text-left text-[10px] uppercase tracking-wider text-slate-400 bg-slate-50">
        ...
        <p className="mt-8 pt-5 border-t border-slate-200 text-[11px] text-slate-400 leading-relaxed">
