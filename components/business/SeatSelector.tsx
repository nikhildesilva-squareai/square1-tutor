...
import { seatRate, tierName } from "@/lib/org";
  ...
  return (
    <div className="max-w-2xl mx-auto rounded-3xl border-2 border-brand/20 bg-white p-6 sm:p-8 shadow-[0_16px_48px_rgba(0,86,206,0.10)]">
      ...
      <div className="flex items-center gap-4 mb-2">
        ...
        <button onClick={() => setSeats((s) => Math.min(50, s + 1))} aria-label="More seats"
          className="w-10 h-10 rounded-xl border border-slate-300 text-slate-700 text-xl font-bold hover:bg-slate-50">+</button>
      </div>
      <input type="range" min={1} max={50} value={seats} onChange={(e) => setSeats(Number(e.target.value))}
      ...
      <div className="rounded-2xl bg-slate-50 border border-slate-200 p-5 mb-5 flex items-center justify-between flex-wrap gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-0.5">{tierName(seats)} tier</p>
          <p className="text-sm text-slate-600">
            <span className="font-bold text-slate-900">${rate}</span>/seat/mo when billing launches
          </p>
        </div>
        <div className="text-right">
          ...
          <p className="text-[11px] text-slate-400">~${annual.toLocaleString()}/yr later · cancel anytime</p>
        </div>
      </div>
      ...
      <Link href={`/business/start?seats=${seats}`}
