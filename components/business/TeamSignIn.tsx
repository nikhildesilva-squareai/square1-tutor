...
// ─────────────────────────────────────────────────────────────────────────────
// Shared B2B sign-in — mirrors the consumer login (email code + Google +
// Microsoft) so the team flow never dead-ends on a single OAuth provider.
//  - OAuth redirects out and back to `next` via /api/auth/callback.
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="space-y-2.5 mb-5">
        <button onClick={() => oauth("google")} disabled={loading}
          className="w-full flex items-center justify-center gap-3 h-12 rounded-xl border border-slate-300 bg-white text-slate-900 font-semibold text-sm hover:bg-slate-50 transition-all disabled:opacity-50">
          <GoogleIcon /> Continue with Google
        </button>
        <button onClick={() => oauth("azure")} disabled={loading}
          className="w-full flex items-center justify-center gap-3 h-12 rounded-xl text-white font-semibold text-sm transition-all hover:brightness-110 disabled:opacity-50"
