...
// Order ↔ chaos particle field. Left half = an ordered grid (pulled back toward
// its lattice), right half = chaotic drift; a divider sits between them.
// Brand-adapted from the 21st.dev "Entropy": configurable colour + transparent
// background so it works on white OR inside a dark panel. Decorative only.

interface EntropyProps {
  className?: string
  size?: number
  /** particle + line colour (6-digit hex) */
  color?: string
  /** wrapper background (default transparent) */
  background?: string
}

export function Entropy({ className = "", size = 400, color = "#3388FF", background = "transparent" }: EntropyProps) {
  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      update() {
        if (this.order) {
          // Ordered particles drift back to their lattice, perturbed by nearby chaos
          ...
          // Chaotic drift, bounded to the right half
    ...
    function animate() {
      ...
      // Centre divider between order and chaos
  ...
  return (
    <div className={`relative ${className}`} style={{ width: size, height: size, background }}>
